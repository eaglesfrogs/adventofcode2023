...
for node in node_hash:
  ...
  if node[2] == 'n':
    if node[3] < 10 and node[0] > 0 and (node[0] - 1, node[1], 'n', node[3] + 1) in node_hash:
      connections.append({ 'conn': (node[0] - 1, node[1], 'n', node[3] + 1), 'val': node_values[node[0] - 1][node[1]] })
    if node[3] >= 4 and node[1] > 0 and (node[0], node[1] - 1, 'w', 1) in node_hash:
      connections.append({ 'conn': (node[0], node[1] - 1, 'w', 1), 'val': node_values[node[0]][node[1] - 1] })
    if node[3] >= 4 and node[1] < cols - 1 and (node[0], node[1] + 1, 'e', 1) in node_hash:
      connections.append({ 'conn': (node[0], node[1] + 1, 'e', 1), 'val': node_values[node[0]][node[1] + 1] })

  if node[2] == 's':
    if node[3] < 10 and node[0] < rows - 1 and (node[0] + 1, node[1], 's', node[3] + 1) in node_hash:
  ...
  if node[2] == 'w':
    if node[3] < 10 and node[1] > 0 and (node[0], node[1] - 1, 'w', node[3] + 1) in node_hash:
      connections.append({ 'conn': (node[0], node[1] - 1, 'w', node[3] + 1), 'val': node_values[node[0]][node[1] - 1] })
    if node[3] >= 4 and node[0] > 0 and (node[0] - 1, node[1], 'n', 1) in node_hash:
      connections.append({ 'conn': (node[0] - 1, node[1], 'n', 1), 'val': node_values[node[0] - 1][node[1]] })
    if node[3] >= 4 and node[0] < rows - 1 and (node[0] + 1, node[1], 's', 1) in node_hash:
      connections.append({ 'conn': (node[0] + 1, node[1], 's', 1), 'val': node_values[node[0] + 1][node[1]] })

  if node[2] == 'e':
    if node[3] < 10 and node[1] < cols - 1 and (node[0], node[1] + 1, 'e', node[3] + 1) in node_hash:
...
while len(pq) > 0:
  ...
  if current_node[0] == target_node_coords[0] and current_node[1] == target_node_coords[1] and current_node[3] >= 4:
